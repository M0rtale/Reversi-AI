...
ICS46_DYNAMIC_FACTORY_REGISTER(OthelloAI, fengkaiq::Old_new, "727↓");

//returns a vector of possible moves
std::vector<std::pair<int, int>> fengkaiq::Old_new::getPossibleMoves(const OthelloGameState& state, bool enemy)
{
...
//1-(x^(2)+y^(2))
float fengkaiq::Old_new::CenterEval(const OthelloGameState& state, int x, int y)
{
...
int fengkaiq::Old_new::evaluate_winning(const OthelloGameState& state)
{
    ...
    int edgeEvaluation = 10 * iTotalMoves;
    ...
    int movesEvaluation = 200 * ((int)(unsigned int)myMoves.size() - ENEMY_MOVE_COUNT) / ((int)(unsigned int)myMoves.size() + ENEMY_MOVE_COUNT);
    //int movesEvaluation = 0 + ENEMY_MOVE_COUNT - ENEMY_MOVE_COUNT;
    int scoreEvaluation = 100 * (myscore - enemyscore) / (myscore + enemyscore);
    //int cornerEvaluation = 0;
    //int cornerEvaluation = 1000 * mycornerCount;
    int cornerEvaluation = 1500 * (mycornerCount - enemycornerCount) / ((mycornerCount + enemycornerCount) == 0 ? 1 : (mycornerCount + enemycornerCount));
    //int cornetMoveEvaluation = 0;
    //int cornetMoveEvaluation = 1000 * myCornetMoves;
    int cornetMoveEvaluation = 0 * (myCornetMoves - enemyCornetMoves) / ((myCornetMoves + enemyCornetMoves) == 0 ? 1 : (myCornetMoves + enemyCornetMoves));
...
std::pair<int, std::pair<int, int>> fengkaiq::Old_new::minimax_recurs(const OthelloGameState& state, bool IPlayBlack, int max, int min, int depth)
{
    ...
    //the following is some alpha beta pruning procedures
...
std::pair<int, int> fengkaiq::Old_new::chooseMove(const OthelloGameState& state)
{
...
//     if(ms_int > 2500) std::cout << "Old_new disqualified... " << (double)ms_int / 1000 << " at score: " << state.blackScore() + state.whiteScore() << std::endl;
//     return ret;
// }

//damn you thornton! why can't thou make it easier for me to analyze my opponent!
//todo: fix this damn piece of abomination
bool fengkaiq::Old_new::isValidMove(const OthelloGameState& state, int x, int y, bool enemy)
{
